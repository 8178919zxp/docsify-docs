...
快捷键（在 IntelliJ IDEA 中输入以下缩写后按 Tab/Enter 键即可自动补全）：

- `psvm`：快速生成 `public static void main(String[] args) {}` 主方法
- `sout`：快速生成 `System.out.println()` 输出语句
...
| 变量   | static | 分配在方法区（静态区）中，所有实例共享同一份存储，不会重复分配内存 |
| 方法   | final  | 子类中将不能被重写                                       |
| 静态代码块 | static | 虚拟机优先加载                                           |
| 类     | final  | 不能被继承                                               |
...
- 二进制：由0和1组成的数字序列。从**JDK 7**开始允许用 `0b` 或 `0B` 前缀字面表示二进制数，例如 `0b01101100`
...
字符常量只可以用于表示一个字符，并且要用**''**(英文的单引号括起来)。
...
字符串常量可以是一个或者多个字符也可以为0个字符，要用**""**(英文的双引号括起来)。
...
在混合运算中，低精度类型会自动提升为高精度类型（如 int 自动转换为 double）；但反过来，若要将高精度类型转换为低精度类型（如 double 转 int），则必须使用强制类型转换
...
System.out.println(b);//输出结果为：asjd
~~~
...
String 对象一旦创建，其内容不可更改。如需"修改"，实际上是创建了一个新的 String 对象，并将变量指向它。可以通过拼接或其他方法来生成新字符串。
...
当然使用**`Arrays类`**中的**`toString`** 方法会返回一个包含数组元素的字符串，我感觉更简单。
~~~java
int[] a={1,2,3,4,5,6,7};
...
实际上 Java 中是有访问权限控制的，就像我们的个人隐私一样，我不允许别人随便查看，只有我们同意的情况下，才能让别人知道我们的名字、年龄等信息。
...
public E get(int index){
        if (index < 0 || index > (size-1))
            throw new IndexOutOfBoundsException("非法位置，合理位置为0~"+(size-1));
        Node<E> node = head;
        while (index-- >= 0){
            node = node.next;
        }
...
通过这种方式，我们就可以通过连接不同的结点形成一颗二叉树了，这样也更便于我们去理解它，首先定义一个类：
...
class Test_TreeNode{
    public static void main(String[] args) {
        ...
        b.left=d;//b的左边指向d
        b.right=e;//b的右边指向e
        System.out.println(a.left.left.element);//要获取D，需要从根节点出发，所以为a的左边的左边的值就是D
    }
}
~~~
...
层序遍历，实际上这种遍历方式是我们人脑最容易理解的，它是按照每一层在进行遍历：
...
| 第一次入队             | 队首 | A    |      |      | 队尾 |
| ---------------------- | ---- | ---- | ---- | ---- | ---- |
| 第一次出队后第二次入队 | 队首 | B    | C    |      | 队尾 |
| 第二次出队后第三次入队 | 队首 | D    | E    | F    | 队尾 |
...
```
7的左边没有，7的右边有一个，所以左边0-1为-1
18的左边最长树枝为3，右边为2，所以3-2为1
```
...
![](assets/平衡二叉树LL型1.png)

首先我们来看这种情况，这是典型的LL型失衡，为了能够保证二叉树的平衡，我们需要将其进行**旋转**来维持平衡，去纠正最小不平衡子树即可。那么怎么进行旋转呢？对于LL型失衡，我们只需要进行右旋操作，首先我们先找到最小不平衡子树，注意是最小的那一个：

![](assets/平衡二叉树LL型2.png)

可以看到根结点的平衡因子是2，是目前最小的出现不平衡的点，所以说从根结点开始向左的三个结点需要进行右旋操作，右旋需要将这三个结点中间的结点作为新的根结点，而其他两个结点现在变成左右子树：

![](assets/平衡二叉树LL型3.png)

右旋以后所有的结点就都保持了1，0，-1的关系。

2.**RR型调整**（左旋）

与LL型相反即可：

![](assets/平衡二叉树RR型1.png)

只需要左旋就行了：

![](assets/平衡二叉树RR型2.png)

旋后直接将多的结点绑定在原本有问题的结点上就符合了平衡二叉树规则。

3.**RL型调整**（先右旋，再左旋）

结构：

![](assets/平衡二叉树RL型1.png)

针对于这种情况，我们需要先进行右旋操作，注意这里的右旋操作针对的是后两个结点：

![](assets/平衡二叉树RL型2.png)
...
和RL型相反，结构：
![](assets/平衡二叉树LR型1.png)

形状是先向左再向右，这就是典型的LR型了，我们同样需要对其进行两次旋转：

![](assets/平衡二叉树LR型2.png)

这里先进行的是左旋，然后再进行的右旋，这样二叉树就能继续保持平衡了。

这样，只需要在插入结点时注意维护整棵树的平衡因子，保证其处于稳定状态，这样就可以让这棵树一直处于高度平衡的状态，不会再退化了。

------

#### 树：红黑树

通过在插入结点时维护树的平衡，这样就不会出现极端情况使得整棵树的查找效率急剧降低了。但是这样是否开销太大了一点，因为一旦平衡因子的绝对值超过1那么就失衡，这样每插入一个结点，就有很大的概率会导致失衡，我们能否不这么严格，但同时也要在一定程度上保证平衡呢？这就要提到红黑树了。
...
那么什么时候需要变色，什么时候需要旋转呢？通过一个简单例子来看看：
...
散列函数也叫哈希函数，哈希函数可以对一个目标计算出其对应的哈希值，并且，只要是同一个目标，无论计算多少次，得到的哈希值都是一样的结果，不同的目标计算出的结果介乎都不同。

先设计一个哈希函数，这里采用的是余数的形式，给了10列：
...
然后设计存入函数，因为存入的哈希值可能余数一样，所以想到用链表来解决这个问题，先给每一列头部按上一个空白的head部方便链接：
...
接着设计判断这个数是否存在在哈希表中的函数和重写toString方法：
...
//获取哈希列表数位数
     ...
     public String toString(){
         ...
         for (int i = 0; i < TABLE_SIZE; i++) {
             ...
             while (head != null){
                 builder.append(i).append("行：").append(head.element).append("->");
                 head = head.next;
             }
             builder.append("\n");
         }
...
class Test_HashTable{
    public static void main(String[] args) {
        ...
        System.out.println(table.contains(20));
        System.out.println(table.contains(30));
        System.out.println(table.contains(110));
...
true
false
0行：90->0行：80->0行：70->0行：60->0行：50->0行：40->0行：30->0行：20->0行：10->0行：0->
1行：91->1行：81->1行：71->1行：61->1行：51->1行：41->1行：31->1行：21->1行：11->1行：1->
2行：92->2行：82->2行：72->2行：62->2行：52->2行：42->2行：32->2行：22->2行：12->2行：2->
3行：93->3行：83->3行：73->3行：63->3行：53->3行：43->3行：33->3行：23->3行：13->3行：3->
4行：94->4行：84->4行：74->4行：64->4行：54->4行：44->4行：34->4行：24->4行：14->4行：4->
5行：95->5行：85->5行：75->5行：65->5行：55->5行：45->5行：35->5行：25->5行：15->5行：5->
6行：96->6行：86->6行：76->6行：66->6行：56->6行：46->6行：36->6行：26->6行：16->6行：6->
7行：97->7行：87->7行：77->7行：67->7行：57->7行：47->7行：37->7行：27->7行：17->7行：7->
8行：98->8行：88->8行：78->8行：68->8行：58->8行：48->8行：38->8行：28->8行：18->8行：8->
9行：99->9行：89->9行：79->9行：69->9行：59->9行：49->9行：39->9行：29->9行：19->9行：9->
...
实际上我们之前介绍的`TreeSet`和`TreeMap`，在我们不指定任何排序规则的情况下，默认也是采用类中Comparable的实现进行排序的，如果我们直接使用一个未实现`Comparable`接口的类型，同样会出现报错
...
既然数组操作都这么方便了，集合操作能不能也安排点高级的玩法呢？那必须的，JDK为我们准备的Collections类就是专用于集合的工具类，比如我们想快速求得List中的最大值和最小值：
...
>注:**尽量一个功能一个方法,**方便代码的利用,减少代码过多的重复!
...
//通过@Data注解，自动生成getter和setter方法，自动生成toString方法，自动生成equals方法，自动生成hashCode方法，自动生成无参构造方法，自动生成全参构造方法
@Data
//通过@NoArgsConstructor注解，自动生成无参构造方法
@NoArgsConstructor
//通过@AllArgsConstructor注解，自动生成全参构造方法
@AllArgsConstructor
...
再创造实现方法的类命名为：`LibraryService`,其中通过对象流写入图书信息到指定文件中，在系统开始阶段通过方法加载文件，并且如果没有文件就指定方法常量为空列表:
...
public class LibraryService {
    ...
    private void LoadBooks(){
        File file = new File("D:\\projects\\Java_Learn\\untitled\\src\\javaSE\\IO_File\\Book.txt");
        if (file.exists()) {
            try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(file))){
                books = (List<Books>) in.readObject();
            }
            catch (IOException | ClassNotFoundException e){
                e.printStackTrace();
            }
    ...
    private void addBook() {
        ...
        book.setName(order.next());
        System.out.println("请输入图书作者：");
        book.setAuthor(order.next());
        System.out.println("请输入图书出版日期：");
        book.setDate(order.next());
        System.out.println("请输入图书价格：");
        book.setPrice(order.nextDouble());
        books.add(book);
    ...
    private Books queryBook(String name) {
    ...
    private void queryBook() {
        ...
        Books book = queryBook(name);
        if (book != null) {
            System.out.println(book.getName()+"："+book.getAuthor()+"  "+book.getDate()+"  "+book.getPrice());
        }else {
            System.out.println("没有此图书！");
            showBooks();
        }
    ...
    private void SaveBooks(){
        showBooks();
        try(ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("D:\\projects\\Java_Learn\\untitled\\src\\javaSE\\IO_File\\Book.txt"))) {
...
复位中断标记后，会立即清除中断标记。在Java 21之前，我们还可以使用`suspend()`和`resume()`用于暂停和恢复线程，但是由于其存在严重弊端，这两个方法已经被移除，这里就不再介绍了，有关线程暂停和恢复最佳方案会在下面的课程中介绍。
...
> 针对于旧版本不推荐使用 `suspend()`去挂起线程的原因，是因为`suspend()`在使线程暂停的同时，并不会去释放任何锁资源。其他线程都无法访问被它占用的锁。直到对应的线程执行`resume()`方法后，被挂起的线程才能继续，从而其它被阻塞在这个锁的线程才可以继续执行。但是，如果`resume()`操作出现在`suspend()`之前执行，那么线程将一直处于挂起状态，同时一直占用锁，这就产生了死锁。
...
它可以判断一个对象是否为此接口的实现或是类的子类，而现在我们有了更多的方式去判断类型：
...
我们可以发现，反射非常暴力，就连被定义为final字段的值都能强行修改，几乎能够无视一切阻拦。

我们来试试看修改一些其他的类型，比如JDK的官方类：
...
**注意：** 在Java 9之后，需要修改启动参数才能对官方类的私有属性的访问权限进行修改，打开IDEA，添加启动参数：`--add-opens java.base/java.util=ALL-UNNAMED`，这里的`java.util=ALL-UNNAMED`需要修改为对应的包名。

实际上，整个ArrayList体系由于我们的反射操作，导致被破坏，因此它已经无法正常工作了，同样的，我们甚至能直接通过反射修改String的值：
...
value.set(str, "World".getBytes());
System.out.println(str);
```

可以看到，我们通过反射强行修改了String内部的`value`字段（存储字符串内容的数组）打破了String为不可变字符串的约束，轻松破坏了Java世界的规则。再次强调，在进行反射操作时，必须注意是否安全，虽然拥有了创世主的能力，但是我们不能滥用，非特殊情况请不用使用反射API。

------

### (Java 9) 模块化机制

在Java 9之前，Java程序主要是以 "包 (package)" 的方式组织代码，但是所有的代码都在一个整体中运行，没有明确的边界。Java 9引入**模块（Module）**的概念，帮助开发者更好地组织、封装和管理大型应用程序。模块化带来的优点：

- **更好的封装**：只暴露需要对外使用的接口，其余内部实现细节隐藏起来。
- **依赖管理**：明确模块之间的依赖关系，避免“依赖地狱”。
- **更快的启动和更小的内存占用**：只加载需要的模块。
- **更易维护和扩展**：大型项目变得更有条理。

可见，模块化机制实际上是为大型项目准备的，而我们目前阶段仅仅还在初学状态，这里可以为大家简单介绍一下项目如何进行模块化开发。我们可以将项目分为多个子模块，不同模块开发不同的内容，然后相互作为依赖使用。

![](assets/模块化1.png)

比如我们可以在`app`模块中将`utils`作为依赖导入：

![](assets/模块化2.png)

此时我们就可以在`app`中直接使用`utils`里面定义的类和方法了，同样可以进行包的导入，就像使用JDK官方提供的类那样。当然，在JavaWeb阶段我们还会学习Maven，它相比我们这里的简单模块化来说，是一个更加强大的依赖管理和模块化工具，使用Maven来引入我们自己的模块或是别人的框架模块会更加简单和方便，现代化企业级项目基本都是基于Maven或Gradle进行开发。不过，无论通过哪种方式引入，实际上都适用于Java9的模块化机制。

项目现在是模块化的了，那么到底如何才能使用Java9的模块化机制呢？模块具有四种类型，我们首先介绍一下**应用程序模块**，在我们自己的项目中，我们可以在某个模块下的源代码根路径下新建一个叫做`module-info.java`的文件，它用于配置模块化信息：
...
这里使用`module`关键字（此关键字仅在模块化声明文件中生效）声明模块的名称，后面需要跟随一个花括号。

声明模块后，我们会发现项目中某些类无法正常使用了：

![](assets/模块化报错.png)

这是因为我们现在使用了模块机制，我们如果需要使用其他模块，需要手动进行导入，默认情况下，包中如果没有创建`module-info.java`文件，那么会按照**未命名模块**进行处理，未命名模块同样可以访问所有其他模块提供的类，这样我们之前写的Java 8代码才能正常地在Java 9以及之后的版本下运行。要在模块化的子模块中导入其他未命名模块，我们只需要使用其他模块的默认名称即可：

```java
module hello.app {
    requires utils;   //其他模块的名称，默认是IDEA中的子模块名称
}
```

通过`requires`关键字来导入模块，此时我们就成功引入了一个未命名模块，其中的类型就可以正常使用了。除此之外，我们也可以让这些模块同样包含一个`module-info.java`文件，并且为其手动指定模块名称：
...
此时我们就可以使用文件中定义的模块名称进行导入了：

```java
module hello.app {
    requires hello.utils;  //输入模块名称
}
```

不过此时依然会出现模块中类无法使用的问题，这是因为模块化之后，我们还需要手动指定哪些包可以供其他模块导入使用才可以，由于我们之前没有使用Java 9的模块新特性，**未命名模块**只能默认暴露给其他未命名的模块和自动模块，应用程序模块无法访问这些类（实际上就是传统Java 8以下的编程模式，因为没有模块只需要导入依赖就能直接用）现在模块化之后 ，我们需要使用`export`关键字来手动暴露哪些运行其他模块访问：

```java
module hello.utils {
    exports com.test.utils;  //输入你想要允许其他模块访问的包
}
```

添加之后，这个包下的所有内容就可以在外部使用了，但是项目中未暴露的包依然是无法被外部模块去使用的。

- 除了我们这里介绍的两种模块之外，还有一个叫做 **系统模块** 它是来自JDK和JRE的模块（官方提供的模块，比如我们之前使用的所有类都算），我们也可以直接使用`java --list-modules`命令来列出所有的模块，不同的模块会导出不同的包供我们使用。其中`java.base`模块包含我们之前使用的核心类如`java.lang`、`java.util`等，这个模块不需要我们手动进行导入，会自动包含。
- 当然，针对于一些可能并不是Java 9以上的模块项目，这种时候就需要做兼容了，默认情况下是直接导出所有的包，可以访问所有其他模块提供的类，不然之前版本的库就用不了了，这种类型的模块我们称为**自动模块**。

我们最后来回顾一下前面讲解的反射机制，我们发现如果我们现在虽然可以依赖了一个模块，但是是没办法直接进行越级反射操作的：

```java
Class<Utils> utilsClass = Utils.class;
Method hello = utilsClass.getDeclaredMethod("hello");
...
这个错误实际上在我们前面的JDK类中进行反射操作也出现了，要允许其他模块使用越级反射操作，我们可以在模块的前面添加`open`关键字来允许其他模块的越级访问操作：

```java
open module hello.utils {
    exports com.test.utils;
    //open也可以单独指定某个包
}
```

此时我们再去使用反射修改权限就没有问题了。而前面修改JDK类遇到的问题也是一样的原因，因为默认情况下JDK的官方模块并未向我们open访问权限，所以无法直接进行越级反射操作。除此之外，模块还支持单独标明接口和模块对于接口的实现，用到`provide`和`uses`关键字，有关具体内容我们会在JavaWeb阶段进一步补充。

------

### 类加载器

我们接着来介绍一下类加载器，实际上类加载器就是用于加载一个类的，但是类加载器并不是只有一个。我们来研究一下Java9之后的类加载机制（Java8之前流程请看旧版课程）

> ClassLoader（类加载器）是Java中的一个重要组件，负责在运行时将类的字节码加载到JVM中。

Java中所有的类都是依靠类加载器进行加载并生成对应的Class对象，类加载器不止一个而是有多个，不同的类加载器的职责也不一样，类加载器默认有以下三种：

- **BootClassLoader**：启动类加载器，用于加载JDK核心模块，如`java.base`、`java.desktop`、`java.logging `等。
- **PlatformClassLoader**：平台类加载器，用于加载一些平台相关模块，也就是一些扩展类，如`java.compiler`、`java.sql`等。
- **AppClassLoader**：应用类加载器，主要用于加载我们自己编写的类，以及其他JDK中没有被上述两个类加载器加载的模块和类。

对于采用模块化的项目来说，在程序启动后，根据不同的模块划分，不同类会在一开始直接分配给对应的ClassLoader进行加载：

![](assets/类加载器1.png)

对于传统非模块化项目来说，在程序启动后，会按照Java8之前的双亲委派机制进行类的加载：

![](assets/类加载器2.png)

当类需要进行加载时，默认会由最下层的AppClassLoader接手，但并不会立即加载类，而是向上传递询问其父类加载器PlatformClassLoader能否进行加载，同样的PlatformClassLoader也不会立即加载，而是继续向上传递到顶层BootClassLoader进行判断，如果顶层可以直接加载这些类（比如JDK核心类就可以直接加载）那么会直接在BootClassLoader中完成类加载，并直接得到对应的Class对象，如果顶层不行，再继续向下让PlatformClassLoader进行加载，如果还是无法加载，最后才会让AppClassLoader尝试加载，比如我们自己编写的类就是由AppClassLoader完成加载的，这样的类加载机制我们也叫**双亲委派机制**。
...
由于早期BootClassLoader是由C++编写的（叫做BootstarpClassLoader），因此在之前无法获取到对应的对象，Java9之后虽然已经变成Java类，但是为了保持统一就依然保持返回`null`作为结果。

通过前面的了解，既然通过ClassLoader就可以用来加载类，那么我们可以自己手动将class文件加载到JVM中吗？先写好我们定义的类：
...
编译后，得到一个class文件，我们把它放到根目录下。因为上面提到的这三种ClassLoader都无法直接加载class文件，因此我们需要编写一个自定义的类加载器来让它支持读取byte数据加载类文件：

```java
//自己定义一个ClassLoader
static class TestClassLoader extends ClassLoader {
    public Class<?> loadClass(String name, String file) throws ClassNotFoundException {
        try(InputStream stream = new FileInputStream(file)) {  //读取磁盘类文件
            byte[] bytes = stream.readAllBytes();
            return defineClass(name, bytes, 0, bytes.length);  //使用父类提供的defineClass通过byte数据加载类
        } catch (IOException e) {
...
public static void main(String[] args) throws ClassNotFoundException {
    ...
    Class<?> clazz = classLoader.loadClass("com.test.Test", "Test.class");
    System.out.println(clazz);
}
```

现在，我们就将此class文件读取并解析为Class了，并且我们自己创建的类加载器，会将AppClassLoader作为父类加载，现在我们就可以对此类进行操作了（注意，我们无法在代码中直接使用此类型，因为它是我们直接加载的，只能通过反射来操作），我们来试试看创建一个此类的对象并调用其方法：
...
- @FunctionalInterface- Java 8 开始支持，标识一个函数式接口（即只有一个抽象方法的接口）。
